...
class SettlementCalculator:
        ...
        for credit in credits:
            while credit > 0 and debits:
                ...
                penalties = SettlementCalculator._counting_of_penalties(
                    credit=credit_paid, overdue_days=overdue_days
                )
    ...
    @staticmethod
    def _counting_of_penalties(credit: Credit, overdue_days: int):
        return (
            SettlementCalculator._calculate_base_penalty(
                credit,
                overdue_days,
            ),
            SettlementCalculator._calculate_additionaly_penalty(
                credit,
            ),
            SettlementCalculator._calculate_percent_penalty(
                credit,
                overdue_days,
            ),
        )
    ...
    def _calculate_base_penalty(
        credit_amount: Decimal, overdue_days: int, penalty_rate=0.5
    ) -> Decimal:
    ...
    def _calculate_percent_penalty(
        amount: Decimal, overdue_days: int, bank_rate: int = 16
    ) -> Decimal:
